...
- **CRITICAL: CoTracker3 input range bug — video tensor was being fed at [0, 1] instead of [0, 255].** Both `NV_CoTrackerTrajectoriesJSON` and the older `NV_CoTrackerBridge` were passing ComfyUI's native [0, 1] IMAGE tensor straight to the CoTracker3 model. The model is trained on [0, 255] float inputs (per the official PyTorch Hub examples — `read_video_from_path` returns uint8 [0, 255] and only `.float()` is called, no normalization). With 255× too-small input, the model sees almost-black frames regardless of actual content, and visibility scores collapse to ~0 on every anchor — which is the exact symptom that surfaced when NV_PreCompWarp tried to use the trajectories directly: "Min visibility per anchor: anchor_0=0.00 ... anchor_10=0.00" on a perfectly-textured close-up face. The bridge node had been masking the bug because its median-displacement averaging across degenerate trajectories produced near-zero warps that looked roughly correct on tight crops. Fix: multiply the video tensor by 255.0 before passing to CoTracker3 in both nodes. Diagnostic prints added to `NV_CoTrackerTrajectoriesJSON` (tracks/visibility shape, dtype, range, mean) so input/output mismatches with future CoTracker versions surface at runtime.

### Added

- **NV_CoTrackerTrajectoriesJSON `debug_overlay` IMAGE output** — renders the input video with each anchor drawn as a colored dot per frame: filled circle when CoTracker visibility ≥ `min_visibility` (raw track trusted), hollow ring + diagonal cross when below threshold (linearly interpolated). Per-anchor distinct color via HSV cycle, anchor index labels, header bar with frame number / visible count / max frame-to-frame jump in pixels. Wire to NV Preview Animation or VHS_VideoCombine to *see* whether the tracker is locked on or losing anchors. Toggleable via `build_debug_overlay` (default ON during debugging).
- **NV_CoTrackerTrajectoriesJSON expanded info string** — adds per-anchor visibility breakdown (vis_avg / vis_min / visible_frames%), frame-to-frame max-anchor-jump percentiles (p50/p95/p99/max + worst frame), and visibility-cohort frame counts ("frames with all anchors visible" / "frames with ≥N/2 visible" / "frames with <2 visible — warp cannot fit similarity"). Surfaces tracker-failure mode at-a-glance instead of requiring downstream residual inspection.

### Changed

- **NV_PointPicker frame nav buttons now partial-queue this node only**, instead of full-queueing the entire workflow. Uses `app.queueOutputNodes([node.id])` (the public API behind right-click → Queue Selected Output Nodes), with a fallback to invoking the canvas command with this node selected. If neither path is available, the buttons refuse to silently full-queue and instead prompt the user to right-click → Queue Selected. Earlier behavior was running every downstream CoTracker / VACE / sampler node on each ⏪⏩ click — a multi-minute graph re-run for what should be a sub-second preview refresh.
- **NV_PreCompWarp: `render_anchors_t0` (static [N,2]) replaced with `render_trajectories` (per-frame [T,N,2]).** The previous static-frame-0 anchor input could only align a moving source actor to a *static* render — but Kling/Seedance renders are themselves moving videos, so as the source pose advanced the residuals climbed monotonically (observed: 77→1185 px on a 150 px torso = total fit failure). The fix solves per-frame `M_t: source[t] → render[t]`, which requires CoTracker-tracked trajectories on both videos. Workflow update: add a second `NV_CoTrackerTrajectoriesJSON` on the render's PointPicker output and wire it to the new `render_trajectories` input. Same anchor-count and same-order requirement as before. Removed the now-dead `_parse_anchors_t0` helper.
- **NV_PromptRefiner Kling system prompts reworded for accuracy.** Previous wording implied separate API budgets for body/refs/negative — Kling 3 Omni actually has a SINGLE 2500-char prompt cap, no `negative_prompt` field. Pipeline inlines an `Avoid: ...` line and reference legends into the same 2500 budget; the 1800/300/400 split is our internal accounting, not three API limits. Reworded both `kling_edit` and `kling_reference` system prompts to reflect this. Verified against Freepik Kling 3 Omni reference + Replicate `kwaivgi/kling-v3-omni-video` model card.
...
- **NV_TemporalMaskStabilizer_V2 — `binarize_output` widget (default ON)** for the bilinear-warp-grey-halo issue. First-test feedback showed soft grey halos around the subject after stabilization even when input was clean binary: cv2.remap uses bilinear interpolation by default, so warping a binary neighbor mask into the current frame's coordinate space introduces fractional values at boundaries; median consensus over `[binary_current, soft_warped_n1, soft_warped_n2]` then produces ~0.5 values at edges where neighbors disagree → visible grey fringe. The existing `binarize_first` widget only controlled INPUT-side binarization and didn't help. Fix: new `binarize_output` BOOLEAN (default True) + `binarize_output_threshold` FLOAT (default 0.5) thresholds the consensus result AFTER the per-frame loop, before final_feather. Matches the "binary in → binary out" expectation for chains starting from NV_MaskBinaryCleanup. Set `binarize_output=False` only if downstream wants soft alpha. Also added `verbose_debug` widget — prints per-call config, per-frame repair status (DROPOUT vs ANOMALY with IoU), progress every 50 frames, timing breakdown (flow+warp vs consensus vs other) to the ComfyUI console.
- **NV_TemporalMaskStabilizer_V2** — motion-compensated short-window temporal mask stabilizer, designed to follow NV_MaskBinaryCleanup in the canonical mask pipeline. Replaces the shelved 2026-04-24 v1 (D-099) which had frame-0 seed poison-pill, infinity-ratio area normalization, 10-px geodesic reach (too small for body-scale subjects), no motion compensation, and OOM on 277f 1080p. V2 explicitly avoids each: short window (3 or 5 frames), per-frame Farneback backward flow → cv2.remap warps neighbor masks into current frame's coordinate space → per-pixel median (or mean) consensus → optional anomaly auto-repair. Streaming compute (one flow at a time, no batch flow precompute). Auto-repair uses consensus-based IoU comparison (current vs neighbor consensus, NOT max of individual IoUs — fixes the empty-current + empty-neighbor false-certification bug from initial review) plus explicit dropout recovery when current frame is empty but any neighbor carries signal. Returns `(stabilized_mask, repaired_flags, info)` — the per-frame repair flag MASK enables downstream debug viz showing exactly which frames were altered. `binarize_first=True` preserves binary intent through the pipeline (re-binarizes after consensus, since linear-interp warp + median reintroduce fractional values). Default `anomaly_iou_threshold=0.35` (Codex's defensible default; lower = stricter trigger = fewer false repairs on valid fast motion). 2 multi-AI rounds: brainstorm consensus design (unanimous #1 priority addition to the suite) + shipped-code review (4 fixes applied — redundant `.astype` copy, `flow_method=none` footgun removed, max(IoU) repair trigger replaced with consensus-based + dropout recovery, `binarize_first` re-binarize after consensus).
- **NV_MaskBinaryCleanup** — new post-process node for converting soft / depth-map-like masks back to clean binary. Pipeline (per frame): `binarize_threshold` → optional `fill_holes` (true binary fill via `scipy.ndimage.binary_fill_holes` — fills any-sized internal holes regardless of kernel, unlike greyscale closing) → optional connected-component cleanup (`min_component_area_px` drops noise, `keep_n_largest` keeps only N biggest CCs by area, cv2.connectedComponentsWithStats) → optional `final_dilate_px` (signed, GPU `mask_erode_dilate`) → optional guided filter refine via `guided_image` IMAGE input + `guided_radius`/`eps`/`strength=0.7` (image-aligned edges; runs on clean binary input so the filter only adjusts edge POSITION instead of redistributing soft mass — sharper than running guided on soft input) → optional `final_feather_px` (Gaussian via `mask_blur` for soft alpha output). Designed to follow soft-mask producers (NV_MaskUnion guided refine, raw SAM3, post-blur) where the output has continuous-valued grey gradients that downstream consumers can't use. Defaults: `threshold=0.5`, `fill_holes=True`, all CC/morph/feather/guided params off → out-of-the-box "make this binary and fill the gaps."
- **NV_MaskUnion seam-repair pipeline** — the naive `torch.maximum` union now has additive options for combining per-region SAM3 masks (face + hair + body) into a seamless whole-subject mask. New widgets (all default OFF, drop-in compatible): `combine_mode` (`max` default | `probabilistic_or` for non-nested layouts), `pre_feather_px` per-input Gaussian, `post_close_px` greyscale closing on the combined union (the main lever — bridges 2-5 px gaps where SAM3 masks disagree on boundary), `post_smooth_px` Gaussian softening, and an optional `guided_image` IMAGE input that runs the existing `guided_filter` (He et al. 2013) on the union edge with auto-radius scaling (`max(4, min(H,W) // 64)`). `clamp_output=False` is rejected with `combine_mode != 'max'` (probabilistic_or math is undefined for out-of-range inputs). `clamp_output` widget kept first to preserve positional `widgets_values` mapping for existing workflows. Plan reviewed in two multi-AI rounds (Codex + Gemini): `sum_clip` cut as option creep, per-input cleanup demoted to single post-union close+smooth (real seams are *between* mask supports, not within them), guided filter kept optional with `strength=0.7` default to prevent over-erosion. SDF-based union deferred to a future `NV_MaskUnionSDF` node for the structural-disagreement case.
- **NV_PromptRefiner `nano_banana_pro` mode** — built-in refiner mode for Google Gemini 3 Pro Image (Nano Banana Pro) image generation/editing prompts. Teaches natural-language role labels for reference images (NO `@image1` tags — Nano Banana Pro doesn't parse them), prose over keyword soup, positive-avoidance language (no negative prompt field), the 5-part scene structure (Subject/Composition/Action/Location/Style) for new generation vs. delta-led edit instructions for editing, identity-preservation via re-description, and text-rendering strengths. Backed by Google AI / Vertex / DeepMind primary docs.
...
- **NV_PromptRefiner — removed broken `google/gemini-3-flash-preview` from model dropdown.** This OpenRouter model ID does not exist in the live catalog (verified 2026-04-28 against `https://openrouter.ai/api/v1/models`). Same pattern as the prior `google/gemini-3-pro-preview` → `gemini-3.1-pro-preview` rename. Users selecting it would have hit a 404. The catalog has `google/gemini-3.1-flash-lite-preview` (already in the dropdown) for video-capable Flash use.
