...
- **NV_SeedanceRefVideo** (MVP) — fork of ComfyUI's ByteDance Seedance 2.0 reference-to-video node with pipeline-friendly inputs. Top-level `prompt` as `force_input` (chainable with PromptRefiner), batched IMAGE tensor input (uniform-sampled to max 9 if oversize), single VIDEO reference, auto-injects `@Image1..N` / `@Video1` handles when prompt lacks them. Validates ref video against pixel budget (409K–927K/frame) and duration envelope (1.8–15.1s). Calls the same BytePlus Seedance 2.0 endpoint as the stock node, uses stock request/response models.
