...
;(function() {
    ...
    ctx.prototype.__applyCurrentDefaultPath = function() {
        if(this.__currentElement.nodeName === "path") {
            ...
            // NV patch: ComfyUI V1 frontend calls fill()/stroke() when current
            // element is a <g> (no preceding beginPath). Skip so the SVG export
            // completes; affected slot fills will be hollow. Warn once so this
            // doesn't silently mask future canvas2svg regressions.
            if (!this.__nv_warned_path) {
                this.__nv_warned_path = true;
                console.warn(
                    "[NV_WorkflowSvg] canvas2svg: suppressed path op on <" +
                    this.__currentElement.nodeName + "> (no beginPath). " +
                    "Fills on that element will be hollow."
                );
            }
            return;
        }
