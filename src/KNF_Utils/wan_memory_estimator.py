...
class NV_WANMemoryEstimator:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "estimate_encode_memory": ("BOOLEAN", {
                    "default": True,
                    "tooltip": "Include VAE encoder feat_cache memory (critical for long videos)"
                }),
                "encode_frames_per_chunk": ("INT", {
                    "default": 0,
                    "min": 0,
                    "max": 10000,
                    "tooltip": "If using NV_ChunkLoader, frames per encode chunk. 0 = full video at once"
                }),
            }
        }
    ...
    def estimate(self, pixel_frames, height, width,
                 ...
                 estimate_encode_memory=True, encode_frames_per_chunk=0):
        """
        ...
        # 2.5. VAE Encoder feat_cache (CRITICAL for long videos!)
        if estimate_encode_memory and use_vace:
            report_lines.append("VAE ENCODER feat_cache (CRITICAL!):")
            # Determine frames being encoded at once
            if encode_frames_per_chunk > 0:
                encode_frames = encode_frames_per_chunk
                report_lines.append(f"  Chunked encoding: {encode_frames} frames per chunk")
            else:
                encode_frames = pixel_frames
                report_lines.append(f"  Full video encoding: {encode_frames} frames at once")

            feat_cache_bytes = estimate_feat_cache_size(vae, encode_frames, height, width)
            report_lines.append(f"  feat_cache size: {format_bytes(feat_cache_bytes)}")

            # Also estimate max safe frames
            # Assume ~8GB available for encoding (conservative for 24GB GPU)
            max_frames_8gb = estimate_max_frames_for_vram(8.0, height, width, vae)
            max_frames_4gb = estimate_max_frames_for_vram(4.0, height, width, vae)
            report_lines.append(f"  Max safe frames @ 8GB available: ~{max_frames_8gb}")
            report_lines.append(f"  Max safe frames @ 4GB available: ~{max_frames_4gb}")

            # For VACE, we encode TWICE (inactive + reactive)
            feat_cache_bytes *= 2
            report_lines.append(f"  VACE total (2x encodes): {format_bytes(feat_cache_bytes)}")

            total_gpu_memory += feat_cache_bytes
            report_lines.append("")
        ...
        # Check feat_cache limits for VACE encoding
        if estimate_encode_memory and use_vace:
            encode_frames = encode_frames_per_chunk if encode_frames_per_chunk > 0 else pixel_frames
            max_safe_frames = estimate_max_frames_for_vram(6.0, height, width, vae)  # Conservative 6GB

            if encode_frames > max_safe_frames:
                report_lines.append(f"  ⚠ VACE ENCODE OOM RISK: {encode_frames} frames > ~{max_safe_frames} safe limit!")
                report_lines.append(f"  → Reduce chunk size or resolution for VACE encoding")
                report_lines.append(f"  → Current: {width}x{height} @ {encode_frames} frames")
                if encode_frames_per_chunk == 0:
                    report_lines.append(f"  → Consider using NV_ChunkLoaderVACE with smaller chunks")
